...
% TABELLEN & LISTEN
% ==============================================================================
\usepackage{booktabs}                   % Professionelle horizontale Linien (\toprule, \midrule, \bottomrule)
\usepackage{longtable}                  % Tabellen über mehrere Seiten
\usepackage{siunitx}                    % S-Spaltentyp für Zahlenausrichtung, \num, \unit etc.
\usepackage{multicol}                   % Zellen über mehrere Zeilen bzw. Spalten
\usepackage{enumitem}                   % Kontrolle über Listen (Abstände, Labels)
\usepackage{pgfplotstable}              % Erlaubt das Erstellen von Tabellen aus .csv Dateien
...
\usepackage{pgfplots}                   % Funktions- und Datenplots über TikZ
    ...
    \usepgfplotslibrary{polar}              % Polarplots (polaraxis-Umgebung)
...
% Hyperref so weit hinten wie möglich, aber immer vor cleveref!
\usepackage{hyperref}                   % Hyperlinks und PDF-Metadaten
%Cleveref als letztes
\usepackage[
    noabbrev,                           % Gleichung 1 anstatt Gl. 1 usw.
    nameinlink,                         % Auch das Gleichung ist ein klickbarer Link. Ohne könnte man nur die Zahl anklicken
    ngerman                             % Explizit nochmal auf deutsche Sprache stellen. Ohne diese Einstellung hatte ich Probleme
    ] {cleveref}                        % Viele Quality of Life Verbesserungen fürs Referenzieren.
